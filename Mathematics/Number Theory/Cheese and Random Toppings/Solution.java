...
public class Solution {
    ...
    static int solve(int N, int R, int M) {
        List<Integer> primeFactors = factors(M);

        Map<Integer, Integer> remainders = new HashMap<>();
        for (int primeFactor : primeFactors) {
            int remainder = computeRemainder(N, R, primeFactor);

            remainders.put(primeFactor, remainder);
        }

        return ChineseRemainderTheorem(M, remainders);
    }
    ...
    static int ChineseRemainderTheorem(int M, Map<Integer, Integer> remainders) {
        int result = 0;
        for (int primeFactor : remainders.keySet()) {
            int remainder = remainders.get(primeFactor);
